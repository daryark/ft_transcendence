import createEngine from "../domain/engine/tetrisEngline";
import { buildGameStats, initGame, type GameState } from "../domain/engine/state";
...
function getRoundLimitMs(room: Room, nextRoundIndex: number): number | null {
    ...
    const limitSeconds = isMatchPointRound && (matchConfig.goldenPoint ?? 0) > 0
        ? matchConfig.goldenPoint
        : matchConfig.winByRounds ?? 0;

    return typeof limitSeconds === "number" && limitSeconds > 0 ? limitSeconds * 1000 : null;
}
...
export default function createMatchService(room: Room, roomService: RoomService) {
    const matchConfig = getMatchConfig(room);
    ...
    function buildGameEndResult(reason: RoundEndReason) {
    ...
    function restartSameRound(previousState: GameState) {
        const nextState = initGame(previousState.rows, previousState.cols, previousState.round);
        copyCarryStats(nextState, previousState);
        startRound(nextState, true);
    }

    function startNextRound(previousState: GameState) {
        const nextState = initGame(previousState.rows, previousState.cols, previousState.round + 1);
        copyCarryStats(nextState, previousState);
    ...
    function evaluate(state: GameState) {
        if (!active || room.status !== "playing" || room.state !== state) return false;
    ...
    function start() {
        ...
        const initialState = initGame(boardHeight, boardWidth, 1);
